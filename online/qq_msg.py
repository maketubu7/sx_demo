...
def read_csv(file,schema):
    path= '/phoebus/_fileservice/users/slmp/shulianmingpin/wa_data/qq_data/'+file
...
def edge_qq_link_group():
    ...
    df = tmp.selectExpr('if(qq1>qq2,qq2,qq1) qq1','if(qq1<qq2,qq2,qq1) qq2',
                ...
                .na.fill('').dropDuplicates(['qq1', 'qq2'])
...
def phone_smz():
    ...
    p1 = read_orc(add_save_path('vertex_person'))
    p2 = read_orc(add_save_path('edge_person_smz_phone_tmp'))
...
def union_jg():
    ...
    files_1 = ['edge_groupcall_sxwa_jg']
    files_2 = ['edge_groupcall_qq_jg']

    for i in range(len(files_1)):
        df1 = read_orc(jg_path+files_1[i])
        df2 = read_orc(tmp_jg_path+files_2[i])
        df = df1.unionAll(df2)
        df.createOrReplaceTempView('tmp')
        sql = '''
                        select max(start_jid) start_jid, max(end_jid) end_jid,start_phone,end_phone,min(start_time) start_time,
                        max(end_time) end_time, sum(call_total_duration) call_total_duration , 
                        sum(call_total_times) call_total_times from tmp
                        group by start_phone, end_phone
                    '''
        res = spark.sql(sql)
        write_orc(res,tmp_jg_path + 'edge_groupcall_tmp_jg')

def test():
    create_tmpview_table(spark,'edge_groupmsg_detail')

    sql = '''
        select * from edge_groupmsg_detail where message like '%阳泉市商业银行%'
    '''

    df = spark.sql(sql)
    write_orc(df,'bank_res')
