...
def edge_person_relation_person_detail(cp):
    ...
    for info in table_list:
        ...
        df = read_orc(add_incr_path(source,cp=cp))
...
def person_month_detail():
    ...
    for info in table_list:
        ...
        detail_path = info['detail']
        ### 每次的最大一次分区
        last_cp = read_orc(add_save_path(target, cp='*', root=root_home)).selectExpr('max(cp)').collect()[0][0]
        detail_res = read_orc(add_save_path(detail_path,cp='*',root=root_home)).where('cp>=2019100000')
        # detail_res.show()
        detail_res.persist()
