...
def udf_format_data_string(data):
    ...
    try:
        ...
        data = data.replace('\r','').replace('\n','').replace('\t', '').replace(' ','').replace(u'︻','').replace(u'　','').strip()
        return data
    except:
        #traceback.print_exc()
        return ''
...
def udf_verify_phonenumber_int(phonenumber):
    ...
    try:
        ...
        if length >= 7 and (length <=11 or (length>11 and phonenumber.startswith('0')))\
...
def udf_timestamp2month_string(timestamp,format="%Y%m0000"):
    try:
        stamp = int(timestamp)
        d = datetime.fromtimestamp(stamp)
...
def add_path(tablename, import_time='*'):
    jz_file = ['phone_owner']
    jz_groupfile = ['dw_evt','voic_busi','vpmn_busi','dw_sms','roam_busi','ods_tb_cdr','elefen_calrec']  ## 三级文件结构 20200602开始改变
    ##源文件地址/phoebus/_fileservice/users/zhjw/sjzl/daml/data/ods/
    ...
    jzgroup_path = '/phoebus/_fileservice/users/jz/jizhen/daml/data/ods/{}/import/{}/*/*'

    for file in jz_file+jz_groupfile:
        if file in tablename:
            return jz_path.format(tablename.upper(), import_time)
    for file in jz_groupfile:
        if file in tablename:
            return jzgroup_path.format(tablename.upper(), import_time)
...
def init(spark,tablename,import_time='',cond='',drop_col=None,if_write=True):
    '''创建临时表'''
...
def init_dw_history(spark,tablename,import_times,if_write=True):
    ...
    for import_time in import_times:
        ...
        try:
            ...
            continue
    write_orc(union_df(dfs), add_save_path(tablename, cp=cp, root=_root_path), mode=write_mode)
...
if __name__ == '__main__':
    print(add_incr_path('edge_groupcall_detail'))
