...
def get_last_cp(tablename,today_cp):
    try:
        last_cp = read_orc(add_save_path(tablename,cp="*", root='person_relation_detail')) \
            .selectExpr('max(cp)').collect()[0][0]
        if last_cp > today_cp:
            last_cp = today_cp
    except:
        last_cp = today_cp
...
def deal_relation_person():
    ...
    for info in table_list:
        ...
        if info.get('root'):
            create_tmpview_table(spark,info['table'],root=root_home)
        else:
            create_tmpview_table(spark,info['table'])
        write_parquet(spark.sql(sql).dropDuplicates(),info['table'])
    for info in table_list:
        dfs.append(read_parquet(info['table']))
    union_df = reduce(lambda x,y:x.unionAll(y),dfs)
    person_df = union_df.dropDuplicates()
    write_parquet(person_df,'person_person')
...
def get_with_airline_detail():
    ...
    write_parquet(df,'airline_detail')
    logger.info('airline_detail parquet down')

    res_person = find_ids_dup(person_df,df,'airlineid')
    write_parquet(res_person,'airline_person')

# 火车明细     
def get_with_trainline_detail():
    ''' 身份证，车次id, 车次，发车日期，座位号 始发站 终点站 '''
    create_tmpview_table(spark,'edge_person_reserve_trainline')
    sql = ''' select sfzh, trianid, cc, fcrq, fz, dz from edge_person_reserve_trainline '''
    df = spark.sql(sql).dropDuplicates(['sfzh', 'trianid']).repartition(200)
    df.persist(StorageLevel.MEMORY_AND_DISK)
    write_parquet(df,'train_detail')
    logger.info('train_detail parquet down')

    ### 找出坐过相同火车的人
    res_person = find_ids_dup(person_df,df,'trianid')
    write_parquet(res_person, 'train_person')
    df.unpersist()
    logger.info('train_person down')

def edge_person_with_airline_travel():
    ## 同航班推理
    df1 = read_parquet('airline_detail')
    df1.persist(StorageLevel.MEMORY_AND_DISK)
    person_df = read_parquet('airline_person')
    ...
    '''
    write_orc(repartition(spark.sql(sql)), add_save_path('edge_person_with_airline_travel'))
    logger.info('edge_person_with_airline_travel down')

def edge_person_with_trainline_travel():
    ## 同火车推理
    df1 = read_parquet('train_detail')
    df1.persist(StorageLevel.MEMORY_AND_DISK)

    person_df = read_parquet('train_person')
    ...
    sql = '''
        ...
        from tmp group by sfzh1, sfzh2
    '''
    write_orc(repartition(spark.sql(sql)), add_save_path('edge_person_with_trainline_travel'))
...
def get_relation_travel_month():
    ...
    for info in with_travel_table:
        last_cp = get_last_cp(info['target'],today_cp)
        create_tmpview_table(spark,info['table'])
        df1 = spark.sql(tmp_sql.format(**info)%last_cp)
        #为了后面的人物图层的计算  这里需做成双向边  便于后面人物关系分的计算
        df = df1.unionAll(df1.selectExpr('sfzh2 sfzh1','sfzh1 sfzh2', 'cp'))
        ## 对每月进行分区统计,两人的同行次数
        df.persist(StorageLevel.MEMORY_AND_DISK)
        cps = [row.cp for row in df.selectExpr('cast(cp as string) cp').distinct().collect() if row.cp >= '2019100000' and  row.cp <= time.strftime("%Y%m0000", time.localtime())]
...
if __name__ == '__main__':
    ...
    # person_df = read_parquet('person_person')
