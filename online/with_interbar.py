...
def get_internetbar_detail():
    #上网明细
    df2 = read_orc(add_incr_path('edge_person_surfing_internetbar_detail')) \
        .selectExpr('sfzh', 'siteid', 'start_time', 'end_time').dropDuplicates()

    write_parquet(df2, 'internetbar_detail')
    logger.info('internetbar_detail down')

    ##相同网吧上网人员
    person_df = read_parquet('person_person') ##同出行的算出的人的直接关系表
    res_person = find_ids_dup(person_df,df2,'siteid')
    write_parquet(res_person,'surfing_person')

def edge_person_with_internetbar_surfing(): 
    
    df1 = read_parquet('internetbar_detail')
    person_df = read_parquet('surfing_person')
    ...
    write_parquet(df5,'internetbar_df5')

    df6 = read_orc(add_save_path('vertex_internetbar')).select('siteid','title','address')
    df5 = read_parquet('internetbar_df5')
...
def get_relation_internetbar_month():
    '''按月明细'''
    last_cp = get_last_cp('relation_withinter_month',today_cp)
...
if __name__ == '__main__':
    ...
    # spark-submit --py-files /home/bbd/workspace/online/commonlib.py --master yarn --deploy-mode  cluster  --name person_with_internetbar --driver-memory 40g --queue bbd_01 with_interbar.py
