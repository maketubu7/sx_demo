...
def vertex_trainline():
    ...
    write_orc(df2,add_save_path('vertex_trainline'))
...
def edge_person_reserve_trainline():
    ...
    create_tmpview_table(spark,'edge_person_reserve_trainline_detail',root='incrdir')
    df1 = spark.sql('select * from edge_person_reserve_trainline_detail')
    create_tmpview_table(spark,'edge_person_enter_trainline_detail',root='incrdir')
    df2 = spark.sql('select * from edge_person_enter_trainline_detail')
    df = df1.unionAll(df2)
    df.repartition(800)
    df2 = df.drop_duplicates(['sfzh','trianid']).selectExpr(*cols)
    write_orc(df2,add_save_path('edge_person_reserve_trainline'))
    logger.info('edge_person_reserve_trainline down')

def edge_person_withtrain_person():
    '''
    同火车出行
    前置
        订火车票
        实名制
    :return:application_1584112390900_31915
    '''
    pass
