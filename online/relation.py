...
def deal_familymate():
    ...
    df = spark.sql(sql)
    write_orc(df,add_save_path('edge_familymate'))
...
def merge_data():
    ...
    sql = '''select sfzh1,sfzh2,concat_ws(';',data) data from(
                   ...
                   (select sfzh1,sfzh2,concat_ws('-',label,zb_score,tb_score,weight_score,substr(cp,0,8)) as data from relation_history_score) tmp
                   group by sfzh1,sfzh2) tmp2'''
    
    df = spark.sql(sql)
    df = df.repartition(800)
    write_parquet(df, 'merge_data')
    logger.info('merge_data down ')

    df = read_parquet('merge_data')
    df.persist()
    last_partition = spark.sql('select max(cp) from relation_history_score').collect()[0][0]
    
    sql = ''' select sfzh1, sfzh2, tb_score,zb_score,weight_score from relation_history_score where cp=%s '''%last_partition
    df2 = spark.sql(sql)
    ...
    df.unpersist()
    write_orc(res, add_save_path("relation_score_res"))
...
def auto_exec():
    ...
    logger.info('next_score(%s,%s)'%(end_month, start_month))
