...
# from sx_demo.online.jg_info import *
spark = SparkSession \
    .builder \
    .config(conf=conf) \
    .enableHiveSupport() \
...
def add_save_path(tablename, cp=''):
    ## hive外表保存地址
    if cp:
        tmp_path = '/phoebus/_fileservice/users/slmp/shulianmingpin/wa_data/zp_new/{}/cp={}'
        return tmp_path.format(tablename.lower(), cp)
    return '/phoebus/_fileservice/users/slmp/shulianmingpin/wa_data/zp_new/{}'.format(tablename.lower())

def add_save_jg_path(tablename):
    ## hive外表保存地址
    return '/phoebus/_fileservice/users/slmp/shulianmingpin/wa_data/zp_jg/{}'.format(tablename.lower())
...
def deal_vertex():
    ...
    for tablename in vertex_table_info:
        ...
        res = spark.createDataFrame(rdd, new_schema) \
            .where('%s != "" and %s != "nan" and %s != "%s" '%(source_table_info[0],source_table_info[0],source_table_info[0],source_table_info[0]))
