...
def phone_filter():
    ...
    if check_all_conds(phone_args):
        ...
        if not is_all:
            ...
            phones = phones.intersect(upload_phone)
            phone_pos = phone_pos.join(phones,'phone','inner') \
                .select(phone_pos.phone,phone_pos.area,phone_pos.cur_time)

        cond_pos_loc = build_cond('area','pos_location','=',phone_args)
