...
def deal_vertex_package():
    ...
    sql = '''
        select packageid, company,orderid, accepttime, inputtime,senderphone, senderaddress, 
        receiverphone,receiveraddress, tablename from package_source_detail
    '''
    df = spark.sql(sql).drop_duplicates(['packageid'])

    write_orc(df,add_save_path('vertex_package',root=save_root))
...
def edge_phone_send_package():
    ...
    write_orc(res,add_save_path('edge_phone_send_package',root=save_root))
...
def edge_phone_receive_package():
    ...
    res = df.drop_duplicates(['phone', 'packageid']).selectExpr(*cols)
    write_orc(res, add_save_path('edge_phone_receive_package',root=save_root))
