...
def edge_groupcall_detail():
    ...
    for cp in cps:
        ...
        for file in detail_files:
            df = read_orc(spark,add_incr_path(file+'_new', cp=cp))
            dfs.append(df)
...
def oversea_call_detail():
    '''提取每天的境外通联数据明细'''
    for cp in cps:
        call = read_orc(spark,add_incr_path('edge_groupcall_detail',cp=cp))
...
def dc_edge_groupcall_detail():
    ...
    df = read_orc(spark,add_incr_path('edge_groupcall_detail')).selectExpr(*cols)
    write_orc(df,add_save_path('edge_groupcall_detail',root=save_root))
...
def dc_edge_groupmsg_detail():
    ...
    df = read_orc(spark,add_incr_path('edge_groupmsg_detail')).selectExpr(*cols)
...
def dc_edge_groupcall():
    ...
    df = spark.sql(sql)
    write_orc(df, add_save_path('edge_groupcall',root=save_root))

    ## 统计出入度 至少有一个大于 1000的
    # macao_phone = read_orc(spark,add_save_path('vertex_phonenumber',root=save_root)).select('phone').where('phone like "853%" and length(phone)=11')
    ...
    # write_orc(macao_phone,add_save_path('macao_phone'))
    # df.unpersist()
...
def dc_edge_groupmsg():
    ...
    df = spark.sql(sql)
    write_orc(df, add_save_path('edge_groupmsg',root=save_root))
...
def compact_par():
    ...
    for cp in cps:
        df = read_orc(spark,add_incr_path("edge_groupmsg_detail", cp=cp))
        if df:
            df = df.coalesce(100)
