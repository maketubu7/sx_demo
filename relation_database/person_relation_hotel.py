...
def vertex_hotel():
    ...
    tmp_sql = '''
        select format_lgdm({lgdm}) lgdm, format_data({qymc}) qiyemc, {yyzz} yyzz, {xxdz} xiangxidizhi,
        '{tablename}' tablename from {tablename} 
        where format_lgdm({lgdm}) != '' and format_data(hote_name) != ''
    '''

    union_df = create_uniondf(spark,table_list,tmp_sql)
    res = union_df.drop_duplicates(['lgdm'])

    write_orc(res,add_save_path('vertex_hotel',root=save_root))
...
def edge_person_stay_hotel_detail_inbord():
    ...
    sql = '''
        ...
        where verify_sfz(coalesce(cert_no,cred_num)) = 1 and format_lgdm(hotel_no) != '' and trim(hote_name) !=''
    '''
...
def edge_person_stay_hotel_detail_oversea():
    ...
    sql = '''
                ...
                where verify_zjhm(cred_num) = 1 and format_lgdm(hotel_no) != '' and trim(hote_name) !=''
            '''
...
def edge_person_stay_hotel_day():
    ...
    dfs = []
    for file in files:
        df = read_orc(spark,add_incr_path(file,cp=cp))
        if df and df.take(1):
...
def edge_person_stay_hotel_detail():
    ...
    ## 国内外住宿明细
    df = read_orc(spark,add_incr_path('edge_person_stay_hotel_detail')).selectExpr(*columns)
    res = df.selectExpr("sfzh", "lgdm", "zwmc", "start_time", "end_time","tablename",
                    "row_number() over (partition by sfzh,lgdm,zwmc,start_time order by end_time desc) num").where("num=1").drop("num")
    res = res.coalesce(200)
    write_orc(res,add_save_path('edge_person_stay_hotel_detail',root=save_root))
    logger.info('edge_person_stay_hotel_detail down')

def edge_person_stay_hotel():
    ##聚合住宿明细
    create_tmpview_table(spark,'edge_person_stay_hotel_detail',root=save_root)
    ...
    write_orc(df,add_save_path('edge_person_stay_hotel',root=save_root))
