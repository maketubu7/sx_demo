...
7）通用模块，文件结尾增加了很多通用的方法，比如初始化原始数据的init
'''
...
def udf_valid_hbh_int(hbh):
    ...
    try:
        ...
        hbh = hbh.strip()
        if re.match('[A-Za-z]{2}', hbh):
            return 1
        elif re.match('[0-9]{1}[A-Z]{1}', hbh):
            return 1
        elif re.match('[A-Z]{1}[0-9]{1}', hbh):
            return 1
        else:
            return 0
    except:
...
def udf_verify_company_name_int(company_name, type=0):
    ...
    try:
        ...
        for sKeyWord in [u'股份', u'公司', u'供销所', u'服务所',u'重庆市',u"有限",u"合作社"]:
            if sKeyWord in company_name:
                return 1
                        
        for sKeyWord in [u'厂', u'场', u'集团', u'培训机构', u'站', u'经营部', u'总公司', u'事务所', u'公安局', u'派出所',
                         u'管理局',u'管理处',u'检察院',u'人民法院',u'统计局']:
...
def udf_format_com_name_string(dwmc):
    pass_key_words = [u'营业部本级',u'（营业部）',u'营业部',u'营业厅',u'本级']
    try:
        for key in pass_key_words:
            dwmc = dwmc.replace(key,"")
        dwmc = udf_format_data_string(dwmc)
        return dwmc
    except:
        return ''
...
def add_path(tablename, import_time='*',is_relate=False):
    ...
    relate_path_total = '/phoebus/_fileservice/users/zhjw/sjzl/daml/data/cdm/dws/his/{}/final/total/{}/*' #关系主题库全量地址
    relate_path_update = '/phoebus/_fileservice/users/zhjw/sjzl/daml/data/cdm/dws/his/{}/final/update/{}/*' #关系主题库增量地址
...
def get_upload_edge(spark,label):
    prop = {
        "url": 'jdbc:mysql://24.2.26.44:3307/graphspace_sx_all?characterEncoding=UTF-8',
        "user": "bbd",
        "password": "bbdtnb",
        "driver": "com.mysql.jdbc.Driver",
        "dbtable": "(SELECT property FROM tb_edge_import_data WHERE label='%s') tmp"%(label)
    }
    return spark.read.json(spark.read.format('jdbc').options(**prop).load().rdd.map(lambda row: row.property))
...
def read_parquet(spark,path_prefix,path):
    '''读 parquet'''
    df = spark.read.parquet(os.path.join(path_prefix, 'parquet/', path))
    return df

def write_parquet(df, path_prefix,path):
    '''写 parquet'''
...
def create_tmpview_table(spark,tablename,cp='',root='extenddir'):
    ...
    try:
        ...
        print('该文件 %s 在cp=%s 没有数据'%(tablename,cp))
        return 0
        pass
    if df and df.take(1):
...
if __name__ == '__main__':
    print(udf_verify_man_int('140502196108052215'))
