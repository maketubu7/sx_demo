...
def add_save_path(tablename, cp=''):
    ...
    if cp:
        ...
        return tmp_path.format(save_root,tablename.lower(), cp)
    return '/phoebus/_fileservice/users/slmp/shulianmingpin/{}/{}/cp=2020'.format(save_root,tablename.lower())
...
def deal_table_count():
    ...
    try:
        ...
        for tablename in vertex_table_info:
            ...
            if df:
                count = df.count()
                logger.info('%s %s'%(tablename,count))
